...
class PagesController extends Controller
{
    ...
    {
        $post = BufferPosting::with('groupInfo.socialGroup','accountInfo.user')->take(100)->get();
        //return response()->json([$post]);

        return view('pages.history')->with('posts', $post);
    }
    ...
    public function searchResult(Request $request)
    {
        if ($request->has('search'))
        {
            ...
            $posts = BufferPosting::with('groupInfo.socialGroup','accountInfo.user')->where('post_text', 'like', '%' . $search . '%')
                ->orWhereIn('group_id', function($query) use ($search)
                {
                    $query->select('id')
        ...
        elseif ($request->has('dataFilter'))
        {
            ...
            else
            {
                $posts = BufferPosting::with('groupInfo.socialGroup','accountInfo.user')
                    ->WhereIn('group_id', function($query) use ($search)
                    {
                        $query->select('id')
                            ->from('social_post_groups')
